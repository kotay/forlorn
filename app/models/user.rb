class User < ActiveRecord::Base
  ...
  INVITATION_CODES = [ENV['INVITATION_CODE'] || 'try']

  # Virtual attribute for authenticating by either username or email
  # This is in addition to a real persisted field like 'username'
  attr_accessor :login

  ACCESSIBLE_ATTRIBUTES = [ :email, :username, :login, :password, :password_confirmation, :remember_me, :invitation_code, :token, :key, :secret, :domain]
  ...
  has_many :gentlemen, :order => "gentlemen.created_at desc", :dependent => :destroy, :inverse_of => :user
